		...
		collector.on("collect", async(reaction) => {
			if(reaction._emoji.name === "◀") {
				...
				if(index1 < 0) {
					...
					desc = `${message.translate("common:SERVERS")}: ${this.client.guilds.cache.size}\n\n` + this.client.guilds.cache.sort((a, b) => b.memberCount - a.memberCount).map((r) => r).map((r, i) => `**${i + 1}** - ${r.name} | ${r.memberCount} ${message.translate("common:MEMBERS")}`).slice(index1, index2).join("\n");
				...
				// Revisar si hay servidores
				if(index1 > this.client.guilds.cache.size + 10) {
					return msg.delete();
				} else if(!index1 || !index2) {
					return msg.delete();
				} else {				
